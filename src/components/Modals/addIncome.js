...
const AddIncome = ({ isIncomeModalVisible, handleIncomeCancel, onFinish, selectedMonth }) => {
  const [form] = Form.useForm();
  
  // When the modal becomes visible or the month changes, reset the date field
  useEffect(() => {
    if (isIncomeModalVisible && selectedMonth) {
  ...
  }, [isIncomeModalVisible, selectedMonth, form]);
      ...
      <Modal
        title={
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            {selectedMonth && (
              <Tag color="blue" icon={<CalendarOutlined />} style={{ marginLeft: '8px' }}>
